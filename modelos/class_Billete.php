...
Class Billete
{
	...
	public function insertar($company,$ruta,$fechaemision,$fesali,$fevuel,$numvuel,$nompasa,$agente,$DNIremitente,$localiz,$precio,$descripcion,$agencia)
	{
		...
		$sql="INSERT INTO remitentes (`DNIremitente`, `nomcompleto`, `tel`, `direccion`, `agentcreaR`, `fecreaR`, `estado`) VALUES ('$DNIremitente','$nompasa',NULL,'Compra de billete','$agente',now(),'1')";
	...
	public function editar($idbillete,$company,$ruta,$fechaemision,$fesali,$fevuel,$numvuel,$nompasa,$DNIremitente,$localiz,$precio,$descripcion,$agencia)
	{
		...
		$sql="UPDATE remitentes SET nomcompleto='$nompasa' WHERE DNIremitente='$DNIremitente'";
		 ejecutarConsulta($sql);
...
/*
	...
	{
		$sql="SELECT idbillete,company,ruta,fechaemision,fesali,fevuel,numvuel,(select nomcompleto from remitentes WHERE DNIremitente=nompasa) as nomcompleto,nompasa as DNIremitente,localiz,precio,descripcion,agencia FROM billetes WHERE idbillete='$idbillete'";
	...
	{
		$sql="SELECT idbillete,company,(select nombreR from rutas WHERE idruta=ruta) as ruta,fesali,fevuel,numvuel,(select nomcompleto from remitentes WHERE DNIremitente=nompasa) as nomcompleto,localiz,precio,descripcion,(select nombre from agencias WHERE idagencia=agencia) as agencia,agentcrea,fechaemision FROM billetes";
		return ejecutarConsulta($sql);
	}
